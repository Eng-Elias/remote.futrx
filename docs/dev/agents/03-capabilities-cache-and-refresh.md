...
| Kimi | Reads `kimi provider list --json`; the plain `provider list` supplies the configured default. Aliases and their overrides are normalized into model records. Only Default mode is exposed; unsupported effort controls are omitted. | Failure of the JSON catalog returns an Auto-only fallback. Failure of only the plain command preserves models and adds a warning. |
